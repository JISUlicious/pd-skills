...
description: >
  ...
  감사, 인과 추론(DiD, 성향점수 매칭, dowhy), 계획된 실험(DOE / ANOVA),
  정성적 RCA(파레토 / 어골도 / 5-Why), 웨이퍼 맵 / 공간적 결함 패턴,
  8D / CAPA 보고 등 전문 워크플로우를 지원합니다. 데이터프레임,
  CSV/Parquet/Excel 분석, 특성 중요도, 드라이버 분석, 수율/불량 조사, 공정
  이상, 변화점, 관리도, 웨이퍼 맵, 파레토·어골도·5-Why, DOE, 8D, 그 외 파이썬
  데이터 사이언스 작업 — 사용자가 "pandas"나 "분석"이라는 단어를 명시하지
  않아도 이 스킬을 사용하세요.
metadata:
  verify:
    - "모델링이나 드라이버 주장에 앞서 모델링 전 진단 4종을 모두 실행했다 (scripts/premodel_audit.py 또는 동등한 인라인 검사)"
    - "필터링·조인·dropna 전후의 행 수를 보고했다"
    - "다중공선성을 감사했고, VIF > 10 항목은 해소했거나 영향을 받는 계수와 함께 명시했다"
    - "결측 처리는 칼럼별로 근거를 밝혔다. 정보성 결측이 있는 데이터셋에 일괄 dropna()를 적용하지 않았다"
    - "R² < 0.4 이거나 방법 간 순위가 엇갈릴 때 선형 기반 드라이버 순위를 SHAP 또는 순열 중요도로 교차 검증했다"
    - "보고한 모든 효과에 표본 크기와 불확실성을 함께 제시했다"
---
...
| 작업 영역 | 읽을 파일 |
|---|---|
| 필수 진단 실행 — 프로브 사용법, 종료 코드, 주의사항 | `scripts/README.md` |
...
**인용하지 말고 실행하세요.** 진단 4종은 `scripts/`에 실행 가능한
프로브로 포함되어 있습니다. 직접 인라인으로 다시 구현하지 말고 여기서
시작하세요. 프로브는 손으로 짠 버전이 자신 있게 헛소리를 내놓게 만드는
퇴화 사례들을 이미 처리합니다 (행 부족으로 계수 행렬의 랭크가 부족한 VIF,
연속형 타깃을 순위화했을 때 1.0으로 포화되는 Cramér's V, 99% 결측 칼럼이
십여 행만으로 완벽한 연관성을 보고하는 경우).

```bash
python scripts/premodel_audit.py <데이터> --target <칼럼>     # 4종 전체, 단일 판정
python scripts/collinearity_probe.py <데이터> --target <칼럼> # 개별 실행도 가능
```

종료 코드 0 = 통과, 1 = 조치가 필요한 발견, 2 = 프로브 실행 자체가 불가.
결과를 읽는 대신 프로그램으로 소비하려면 `--json`을 추가하세요. 각
프로브가 무엇을 단언하는지, 공용 플래그, 그리고 출력 해석을 바꾸는
주의사항(ID 칼럼, 구조적 부재, n/p 비율)은 `scripts/README.md`를
읽으세요. 아래 참조 파일들은 *방법*을 설명하고, 프로브는 그 방법의 권위
있는 구현입니다.
