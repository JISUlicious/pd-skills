...
분석보다 먼저 실행하십시오. 쌍별 ρ는 다중 방향의 중복성을 놓칩니다. 피처 A가
다른 모든 피처와 개별적으로 ρ < 0.3을 가질 수 있지만, 그중 세 개의 완벽한
선형 결합일 수 있습니다(VIF = ∞). Ames Housing 예시: `Gr Liv Area`는 적당한
쌍별 ρ 값을 가졌지만 VIF = ∞였는데, 이는 정확히 `1st Flr SF + 2nd Flr SF + Low
Qual Fin SF`와 같기 때문입니다.
