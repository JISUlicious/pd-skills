...
**FMEA로 루프를 닫으세요.** 발생/유출 분리는 곧 pFMEA 업데이트의
입력입니다 — 분석을 보고서에 방치하지 마세요. 검증된 발생 원인은 그
실패 모드의 **발생도(Occurrence, O)** 등급을, 유출 원인은 **검출도
(Detection, D)** 등급을 갱신합니다(EWMA 차트를 추가했으면 개선, 아무것도
없었으면 악화). **RPN = S × O × D**(심각도 불변)를 재계산하고 D6/D7
조치 후 떨어졌는지 확인하세요 — RPN을 움직이지 않는 시정 조치는 리스크를
다루지 않은 것입니다. 이것이 감사자가 찾는 D7 산출물이며 이미 한 작업에서
자연히 나옵니다.
