...
중요도 모델을 적합시키기 전 두 가지 점검이 필수입니다. 몇 초밖에 걸리지
않지만 가장 흔한 무음 실패 — 공선 예측 변수에 의해 결정되는 순위, 결측
제거에서 살아남은 행에 의해 결정되는 순위 — 를 막아줍니다.
...
지속시간, 매출에서 흔함)은 OLS β에 대한 고-꼬리 관측의 영향력을 부풀리고
이분산 잔차를 만듭니다. 해결책은 한 줄입니다:
...
다룹니다. **모델링에서만** 적용되는 추가 규칙 세 가지가 있고, 위반하기
쉽습니다:
...
감쇄가 0.05 초과이고 **동시에** 결측이 정보를 가지면
(`miss_target_assoc` ≥ 0.05), 지시자+대체 패턴을 사용하세요 — 지시자가
대체로 지워지는 결측 신호를 포착합니다.

순수하게 무작위인 결측 (`miss_target_assoc` ≈ 0)에 대해서는, 단순한
중앙값 대체로 충분합니다. 지시자+대체는 지시자가 분산을 흡수하기 때문에
원본 칼럼의 β를 미세하게 *악화*시킬 수 있습니다(R² 보존은 더 낫지만).
실제로 데이터에서 정보를 갖는 패턴에 맞는 방식을 선택하세요 — 반사적으로
지시자를 추가하지 마세요.
...
중요도를 부풀립니다 (분기 후보가 더 많을수록 이길 기회가 더 많음). 이진
피처가 진정한 드라이버여도 10-수준 범주형이 이를 압도할 수 있습니다.
**Gain을 단독으로 보고하지 마세요.**
...
- **< 0.5:** 메서드가 불일치 → 비선형성, 피처 상호작용, 또는 어느 한
  메서드가 오도되었다고 의심. 순위를 발표하기 전에 조사.
...
| 범주형이 존재하는데 수치형만 사용한 VIF 보고 | VIF ≈ 1인 수치형 칼럼이 범주형에 무음으로 결합 (예: Ames `Garage Yr Blt` ↔ `Garage Finish` η² = 0.998) — β가 의미 있어 보이지만 해석 불가 | `data-exploration.md` § 6의 `mixed_type_vif()`와 `cross_type_binding()` 실행; max 설계 행렬 VIF > 10 또는 쌍별 η² > 0.7인 모든 소스를 공선으로 처리 |
