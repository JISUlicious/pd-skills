# 다중공선성 진단 — VIF, 혼합 타입, 클러스터 대표 선택

Explore 단계의 다중공선성 기계장치. 쌍별 상관은 다중 방향 중복을
놓칩니다. 이 감사들은 그것이 드라이버 분석이나 상관 해석을 오염시키기
전에 잡아냅니다. **선택 사항이 아니며 모델링 시점으로 미룰 수 없습니다**
— Explore 중에 실행하세요.

## 목차

- 다중공선성 감사 (VIF) — 수치형, 모든 데이터셋에서 실행
- 혼합 타입 다중공선성 — 범주형이 존재할 때 (η² / Cramér's V)
- 클러스터 대표 선택 — 5-우선순위 선택기

이 파일을 읽는 경우: VIF / 다중공선성, "상관된 피처 중 무엇을 유지할까",
수치형+범주형 혼합 중복, 또는 `SKILL.md`가 요구하는 모델링 전 공선성
감사. `data-exploration.md` Step 6과 `rca-commonality.md`에서 호출됩니다.

**읽는 대신 실행하려면** 포함된 프로브를 사용하세요. 아래 내용을 모두
구현하고 있으며 퇴화 사례까지 이미 방어합니다:

```bash
python scripts/collinearity_probe.py <데이터> --target <칼럼>      # 세 단계 전체
python scripts/cluster_representative.py <데이터> --target <칼럼>  # 클러스터 해소
```

이 파일의 코드는 참조 구현이자 각 단계가 존재하는 *이유*에 대한
설명입니다. 방법을 변형해야 할 때 — 사용자 정의 점수 함수, 다른 클러스터링
규칙, 더 큰 파이프라인 안에 검사를 내장하는 경우 — 이 코드를 활용하세요.
표준 감사에는 프로브를 실행하면 됩니다.
